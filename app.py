...
def detect_role(job_text):
    # Get first non-empty line (usually the job title)
    ...
    # Remove company/location after separators
...
def ats_recommendation(score):
    if score >= 90:
        return "Excellent", "✅ Excellent ATS score. Your resume is ready to apply."

    elif score >= 80:
        return "Good", "🟢 Good ATS score. Resume generation is optional."

    elif score >= 70:
        return "Fair", "🟡 Fair ATS score. Consider generating an optimized resume."

    elif score >= 60:
        return "Poor", "🟠 Low ATS score. Generating an optimized resume is recommended."

    else:
        return "Very Poor", "🔴 Very low ATS score. Generate a new ATS-optimized resume before applying."
...
if st.button("Analyze Resume"):
    ...
    if not st.session_state.resume.strip() or not st.session_state.job.strip():
        ...
        rating, recommendation = ats_recommendation(score)
        ...
        if matched_skills:
            ...
            st.success("No matched skills found.")
